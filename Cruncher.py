...
class Cruncher:
    ...
    def filter(line, hdr, limit, lowHigh):  #NB: 'line' has NO header character, starts with isoDate
        ...
        if len(line) < 19:  # 19 == length of iISO date string
            return False
        if hdr == "H":
            calc = 24 * (int(line[8:10]) - 1) + int(line[11:13]) #24*(day-1) + hr
        else:
            calc = int(line[8:10]) - 1 #day
        return (calc > limit) == lowHigh
    ...
    def validate(txt, sc, validHdrs, msgOnly):
        ...
        for c in validHdrs:
            if hdr == c:
                validHeader = True
        if not validHeader:
            sc.createAndLogMessage("Invalid header")
            return False
        
        # next validate date format (but not values)
        testDate = txt[1:20]
        format_string = '\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d'
        isISODate = re.match(format_string, testDate)
        if not isISODate:
            sc.createAndLogMessage("Invalid date format")
            return False
        ...
        if hdr == 'D':
            matchNum = g.NUM_DITEMS
        else:
            if hdr == 'H':
                matchNum = g.NUM_HITEMS
            else:
                matchNum = g.NUM_RITEMS
    ...
    def adjustfName(iso):    
        ...
        if sli == "01T00":
            ...
            smn = str(mn)
            if mn < 10:
                smn = '0' + smn # leading zero when needed
            isoReturn = str(yr) + '-' + smn
        else:
            isoReturn = iso[0:7]
        return isoReturn
    ...
    def riseFall(hourlyPress):
        ...
        sumXY = sumXY / 1180.0 # => slope of best fit line: 1180 assumes 24 hourly results, but doesn't matter as
        if sumXY < 0.0:
            rv == -1
        else:
            rv = 1
